...
nghiệm (N=60 scramble random-state cho so sánh cơ sở, kiểm tra độ mạnh
mẽ trên N=100 scramble random-move, N=60 cho quét Pareto) cho thấy việc
thiên vị heuristic tìm kiếm F2L làm tăng độ trùng lặp trigger từ mức cơ
...
(Mục~\ref{sec:hli-conformity}) nhưng chỉ có OLL 2-look (2 thuật toán
sinh, Sune/Anti-Sune, thay vì đầy đủ 57 case); sự bất đối xứng này tự
nó có giá trị thông tin cho việc đo lường Pattern-conformity
(Mục~\ref{sec:method}).

Ba hướng nghiên cứu liên quan (Mục~\ref{sec:related}) hiện chưa giao
nhau: các solver Rubik's Cube tối-ưu-kiểu-máy-tính (Mục~2.1) không
quan tâm đến khả năng diễn giải; lập kế hoạch AI nhận biết con người
(Mục~2.2) đã hình thức hóa explicability nhưng mới chỉ áp dụng cho các
bài toán lập kế hoạch tác vụ ký hiệu quy mô nhỏ, chưa cho một miền tìm
kiếm tổ hợp quy mô lớn như Rubik's Cube; cộng đồng AI game (Mục~2.3) đã
phát triển phương pháp luận đánh giá tính giống-người bằng giám khảo
con người mù (BotPrize) nhưng chưa từng áp dụng cho một tác nhân giải
câu đố tổ hợp. Hệ thống dạy học CFOP gần nhất (ALLURE, Mục~2.4) chỉ bao
phủ mục tiêu con Cross và không báo cáo một đánh giá giống-người mù có
thể so sánh được. Bài báo này lấp khoảng trống đó bằng cách kết hợp cả
ba hướng: một chỉ số giống-người hình thức hóa theo ngôn ngữ explicability,
một cơ chế tìm kiếm tối ưu hóa rõ ràng cho chỉ số đó, và một nghiên cứu
giám khảo con người mù để kiểm chứng cả hai.
...
cụ thể cho lập kế hoạch tác vụ robot bằng cách học một hàm gán nhãn từ
dữ liệu huấn luyện, khác với cách tiếp cận đo lường trực tiếp, không
cần học, mà chúng tôi dùng cho các thành phần của HLI (Mục~\ref{sec:method}).
...
Trong kỳ thi năm 2012, hai bot -- UT\textsuperscript{2} (Đại học Texas
tại Austin) và MirrorBot (Mihai Polceanu) -- lần đầu tiên vượt qua
ngưỡng $50\%$ về tỷ lệ được chấm là "người thật", thậm chí còn cao hơn
tỷ lệ chấm cho chính những người chơi thật tham gia kỳ thi năm đó, cho
thấy một tác nhân tự động được tối ưu hóa rõ ràng cho việc "trông giống
người" có thể vượt qua chính con người trên chính thước đo giống-người
đó. Một khảo cứu gần đây
...
cho PLL; các thuật toán sinh Sune/Anti-Sune cho OLL 2-look) thay vì rơi
vào tìm kiếm không cấu trúc. Toàn bộ $21/21$ case PLL trong bảng đã
được kiểm chứng tự động khi module được import lần đầu, thông qua
\texttt{verify\_and\_build\_table()} (\texttt{solver/pll\_algorithms.py});
chúng tôi xác nhận lại điều này bằng cách chạy trực tiếp hàm này, thu
được đúng $21$ tên case đã kiểm chứng (\texttt{Aa, Ab, E, F, Ga, Gb, Gc,
Gd, H, Ja, Jb, Na, Nb, Ra, Rb, T, Ua, Ub, V, Y, Z}).
...
\ref{app:triggers}). Đây là một kho hạt giống (seed corpus) được tuyển
chọn thủ công từ tri thức phổ biến trong cộng đồng speedcubing, không
suy ra từ thống kê n-gram trên log solve người thật; một bản mở rộng
nên thay thế/bổ sung kho này bằng cách thống kê trực tiếp từ log solve
thật (ví dụ file .log của CSTimer) để giảm thiên vị do tự chọn corpus.
...
thiên vị (tương thích ngược hình thức: chúng tôi đã xác nhận cả $176$
unit test có sẵn từ trước đều pass không đổi với $\lambda=0$). Tăng
...
\textbf{Sinh scramble.} Kết quả chính dùng scramble random-move (20
nước, không lặp mặt liên tiếp) để nhất quán giữa Experiment~1--2. Chúng
tôi cũng cài đặt thêm một bộ sinh scramble kiểu WCA \emph{random-state}
(random walk dài để xấp xỉ phân bố trạng thái đều, sau đó nghịch đảo
một lời giải Kociemba gần-tối-ưu của trạng thái đó — đảm bảo scramble
cần số nước gần với số nước tối ưu để hoàn tác, khớp với phương pháp
sinh scramble thi đấu thật) và đã xác nhận kết quả nhất quán giữa hai
bộ sinh (Mục~\ref{sec:discussion}).
...
lần chạy kiểm tra độ mạnh mẽ random-move, và 2000--2059 cho
Experiment~2) để tái lập được. Chúng tôi giữ nguyên toàn bộ số liệu
trên phần cứng đơn-lõi này cho bản thảo hiện tại; tỷ lệ timeout quan
sát được ($20$--$22\%$, Mục~\ref{sec:pathological}) là một đặc tính
riêng của thiết lập phần cứng/ngân sách node cụ thể này và nhiều khả
năng sẽ thấp hơn trên phần cứng nhiều lõi/nhanh hơn -- đây là một giới
hạn đã biết của kết quả, không phải một thuộc tính phổ quát của
Trigger-Biased Search, và nên được nêu rõ nếu công bố chính thức
dùng phần cứng khác.
...
Bảng~\ref{tab:exp1} báo cáo kết quả trên $N=60$ scramble random-state
kiểu WCA (Mục~\ref{sec:setup}: random walk dài, nghịch đảo lời giải
Kociemba gần-tối-ưu; seed 5000--5059), được chọn làm kết quả chính vì
khớp sát hơn với phương pháp sinh scramble thi đấu thật. Nhóm A dùng
ngân sách node F2L đã giảm (40.000 node/độ sâu, từ mặc định 120.000)
và timeout 20 giây thực cho mỗi lần giải để phù hợp với phần cứng đánh
giá đơn-lõi của chúng tôi; $20\%$ scramble ($12/60$) vượt quá timeout
này và bị loại khỏi thống kê theo cặp bên dưới. Trong số $48$ scramble
được cả hai nhóm giải, kiểm định Wilcoxon signed-rank xác nhận Nhóm A
cần số nước nhiều hơn có ý nghĩa so với Nhóm B ($W=0$,
$p=1,61\times10^{-9}$, $r=0,87$, effect size rất lớn), trong khi theo
cấu trúc chỉ Nhóm A có HLI khác 0. Chúng tôi cũng chạy lại cùng phép so
sánh trên $N=100$ scramble random-move làm kiểm tra độ mạnh mẽ
(robustness check) — kết quả gần như giống hệt, bao gồm một hiện
tượng tìm kiếm không hội tụ đáng chú ý ($22\%$ timeout); xem
Mục~\ref{sec:pathological} và~\ref{sec:validity} để thảo luận chi tiết
về kiểm tra này.

\begin{table}[h]
\centering
\caption{Nhóm A (AI-CFOP) so với Nhóm B (Kociemba), N=60 scramble
random-state kiểu WCA, 48 cặp thành công (Wilcoxon $p=1,61\times10^{-9}$,
$r=0,87$ cho khác biệt số nước)}
\begin{tabular}{lcc}
\toprule
...
$\lambda$ ($0$; $0,5$; $1,0$); chúng tôi chưa xác nhận xu hướng này ổn
định trên cỡ mẫu lớn hơn hoặc các mức $\lambda$ trung gian (ví dụ
$0,25$; $0,75$), và xem đây là một hạn chế cần làm rõ thêm ở công việc
tiếp nối (Mục~\ref{sec:future}) trước khi khẳng định chắc chắn trong
một bản công bố chính thức. Một phân tích hình thức về việc chính xác
mất bao nhiêu tính admissible được trình bày dưới đây.
...
Trong lần chạy kiểm tra độ mạnh mẽ dùng $N=100$ scramble random-move
(Mục~\ref{sec:validity}), $22\%$ scramble ($22/100$) không hoàn thành
trong ngân sách 20 giây thực ngay cả với ngân sách tìm kiếm đã giảm
còn 40.000 node/độ sâu — đáng chú ý, một số thất bại như vậy xảy ra
liên tiếp nhau (ví dụ seed 1059--1061 trong lần chạy của chúng tôi),
gợi ý bộ sinh scramble random-move có thể tạo ra các chuỗi cấu hình
F2L tương tự nhau về cấu trúc, mang tính đối kháng, thay vì độ khó độc
lập đồng phân bố (i.i.d.). Một seed cụ thể (1013) bị treo hơn 5 phút
ngay cả với ngân sách đã giảm, trước khi chúng tôi đưa vào một timeout
thực cứng (dựa trên \texttt{SIGALRM}) trong bộ khai thác đánh giá.
Chúng tôi diễn giải điều này là bằng chứng cho thấy heuristic A* có bổ
sung penalty theo từng cặp dùng cho F2L (Mục~\ref{sec:setup}) đôi khi
bị dẫn vào các vùng tìm kiếm không hiệu quả một cách bệnh lý — một giới
hạn về độ tin cậy quan trọng, khác biệt với, nhưng có thể bị khuếch đại
bởi, cơ chế thiên vị trigger ở Mục~\ref{sec:method}. Lưu ý cả lần chạy
chính (random-state, Bảng~\ref{tab:exp1}) lẫn lần chạy kiểm tra này đều
dùng $\lambda=0$ xuyên suốt (không áp thiên vị trigger cho giai đoạn
F2L); chúng tôi đo riêng mối quan hệ giữa $\lambda$ và tỷ lệ thất bại
hội tụ dùng dữ liệu Experiment~2 (Mục~\ref{sec:discussion}), phát hiện
— trái với kỳ vọng ban đầu — rằng $\lambda$ cao hơn liên quan đến
\emph{ít hơn}, không phải nhiều hơn, các thất bại hội tụ trên tập dữ
liệu đó.
...
\begin{itemize}
  \item \textbf{Tìm kiếm không hội tụ}: như đã báo cáo ở
    Mục~\ref{sec:pathological}, $22\%$ scramble trong lần chạy kiểm tra
    độ mạnh mẽ random-move ($N=100$) và $20\%$ scramble trong lần chạy
    chính random-state ($N=60$, Bảng~\ref{tab:exp1}) không hoàn thành
    trong timeout 20 giây ở thiết lập phần cứng/ngân sách đánh giá của
    chúng tôi; kết quả có điều kiện trên các scramble đã hoàn thành, có
    thể không phải là mẫu con đại diện (ví dụ nếu độ khó tương quan với
    tiềm năng trigger-overlap).
  ...
  \item \textbf{Độ tin cậy liên giám khảo}: từng giám khảo cá nhân có
    độ đồng thuận khiêm tốn (ICC(2,1)$=0,26$, dưới ngưỡng $0,4$ thường
    dùng làm mốc "đồng thuận chấp nhận được"), nhưng nhờ cỡ mẫu
    $N=28$ người chấm, điểm trung bình dùng cho các phân tích tương
    quan ở Mục~\ref{sec:exp3} đạt độ tin cậy cao (ICC(2,k)$=0,91$) --
    tương quan RQ-V1 nên được diễn giải như tương quan của một
    \emph{điểm trung bình ổn định}, không phải của phán đoán từng cá
    nhân đơn lẻ.
  \item \textbf{Thiên vị kho ngữ liệu}: kho finger-trick $T$ được tuyển
    ...
    trigger khác.
  \item \textbf{Sinh scramble (đã kiểm tra)}: Bảng~\ref{tab:exp1} (Mục
    \ref{sec:results}) dùng bộ sinh scramble kiểu WCA random-state
    (Mục~\ref{sec:setup}) làm kết quả chính, vì khớp sát hơn với
    phương pháp sinh scramble thi đấu thật. Chúng tôi cũng chạy lại
    Experiment~1 trên $N=100$ scramble random-move (seed 1000--1099,
    bộ sinh dùng cho Experiment~2) như một kiểm tra độ mạnh mẽ
    (robustness check). Kết quả nhất quán giữa hai bộ sinh (78\% so với
    80\% tỷ lệ giải thành công; số nước trung bình 67,2 so với 66,3;
    HLI trung bình 0,655 so với 0,658), cho thấy kết quả chính không
    phải là hiện tượng giả tạo do lựa chọn bộ sinh scramble cụ thể. Bộ
    random-move này cũng là nơi chúng tôi quan sát thấy hiện tượng tìm
    kiếm không hội tụ đáng chú ý ($22\%$ timeout, xem
    Mục~\ref{sec:pathological}); chúng tôi chưa quan sát cụm thất bại
    tương tự trên $N=60$ mẫu random-state (tỷ lệ timeout thấp hơn,
    $20\%$), nhưng cỡ mẫu nhỏ hơn khiến việc so sánh trực tiếp về tính
    ``cụm'' của thất bại giữa hai bộ sinh chưa đủ mạnh để kết luận chắc
    chắn.
...
Hiện tại Pattern-conformity $P(\pi)$ chỉ áp dụng cho OLL/PLL, vì đây là
hai giai đoạn duy nhất có bảng case-sang-thuật-toán hữu hạn, có tên
trong hệ thống của chúng tôi; Cross và F2L được đo gián tiếp qua
Trigger-overlap thay vì Pattern-conformity thật sự. Xây dựng một bảng
case F2L chuẩn (ví dụ theo hệ thống 41-case phổ biến trong cộng đồng
speedcubing) và một cơ chế nhận diện case tương tự
\texttt{verify\_and\_build\_table()} đã dùng cho PLL sẽ cho phép thay
thế một phần Trigger-overlap bằng Pattern-conformity thật sự cho F2L,
làm HLI phản ánh chính xác hơn cấu trúc của giai đoạn này.

\paragraph{Cơ chế thiên vị thay thế.} Trigger-Biased Search hiện dùng
một siêu tham số $\lambda$ cố định, chọn thủ công. Công việc tiếp nối
có thể so sánh cơ chế này với các phương án khác, ví dụ: tinh chỉnh
trực tiếp trọng số của pattern database dùng cho F2L thay vì cộng thêm
một số hạng bonus riêng biệt, hoặc học trọng số \texttt{bonus(path)} từ
dữ liệu solve người thật (ví dụ hồi quy tần suất trigger quan sát được
trong log CSTimer) thay vì ấn định $\lambda$ bằng tay -- qua đó biến kho
hạt giống $T$ (Phụ lục~\ref{app:triggers}) từ một danh sách tuyển chọn
thủ công thành một mô hình được học trực tiếp từ hành vi người giải
thật.
...
nghiệm ($N=60$ scramble random-state, kiểm tra độ mạnh mẽ trên $N=100$
...
\newblock Trong \emph{Proceedings of the IEEE International Conference on
...
\label{app:triggers}
Bảng~\ref{tab:triggers} liệt kê đầy đủ kho hạt giống $T$ dùng để tính
Trigger-overlap $O(\pi)$ và số hạng \texttt{bonus(path)} trong
Trigger-Biased Search (Mục~\ref{sec:hli-trigger},~\ref{sec:method}),
trích trực tiếp từ \texttt{research/finger\_tricks.py}. Mỗi trigger
được liệt kê ở dạng "thuận" (R-based); hệ thống tự sinh thêm biến thể
mirror (L-based, hoán đổi $R \leftrightarrow L$) khi khớp mẫu.
