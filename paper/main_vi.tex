...
sự \citep{mcaleer2018} giới thiệu Autodidactic Iteration, một tác nhân
học tăng cường có thể học giải cube mà không cần tri thức con người có
sẵn (độ dài lời giải trung vị $\approx$30 nước, cao hơn mức tối ưu 20
nước). Công trình tiếp nối DeepCubeA \citep{agostinelli2019} kết hợp
hàm giá trị đã học này với tìm kiếm A* có trọng số theo lô, giải được
100\% scramble thử nghiệm và tìm ra đường đi ngắn nhất 60,3\% số lần
-- tức đạt độ dài lời giải gần tối ưu. Không phương pháp nào trong số này tạo ra kế hoạch
...
một nghiên cứu pilot gần đây \citep{vanacore2025} (đã công bố bình
duyệt tại IEEE ICHI 2025) phân tích các pattern tương tác của học sinh
...
$|T|=11$ finger-trick phổ biến của con người
\citep{jperm-fingertricks} (ví dụ "sexy move"
...
của khoảng cách tự nhiên phụ thuộc vào trọng số. Phân tích này tái lập
được đầy đủ qua lệnh \texttt{python3 -m research.weight\_sensitivity},
script tự đối chiếu kết quả với các con số nêu trên.
...
$20\%$ scramble ($12/60$) không đạt trạng thái đã giải trong ngân
sách này (10 case timeout thực sự, cộng 2 case dừng lại ở F2L dở
dang trước khi chạm mốc 20 giây) và bị loại khỏi thống kê theo cặp
bên dưới. Trong số $48$ scramble được cả hai nhóm giải,
kiểm định Wilcoxon signed-rank xác nhận Nhóm A cần số nước nhiều
hơn có ý nghĩa so với Nhóm B ($W=0$, $p=1,61\times10^{-9}$, $r=0,87$,
effect size rất lớn; chênh lệch trung bình $45,7$ nước, CI 95\% bootstrap
$[43,3\,;\,48,9]$, BCa, $9999$ lần lấy mẫu lại), trong khi theo cấu
trúc chỉ Nhóm A có HLI khác 0 (trung bình $0,658$, CI 95\%
$[0,629\,;\,0,682]$). Chúng tôi cũng chạy lại phép so sánh này trên
$N=100$ scramble
...
lặp trigger ($W=29,0$, $p=8,25\times10^{-8}$, $r=0,75$, effect size rất
lớn) và số nước ($W=209,5$, $p=0,0014$, $r=0,45$, effect size trung bình);
trên tập con ghép cặp này, chênh lệch trung bình theo cặp là $+0,163$
độ trùng lặp trigger (CI 95\% $[0,122\,;\,0,206]$) và $+1,75$ nước (CI
95\% $[0,59\,;\,2,86]$, rộng hơn đáng kể so với ước lượng điểm nếu so
tương đối, nhất quán với việc hiệu ứng số nước nhỏ và nhiễu hơn hiệu
ứng trigger-overlap ở mức từng scramble; CI 95\% bootstrap, BCa, $9999$
lần lấy mẫu lại).
...
con người ($\rho=0,605$, $p=0,0017$, CI 95\% bootstrap $[0,19\,;\,0,84]$,
BCa, $9999$ lần lấy mẫu lại), cho thấy nó vẫn là thành phần
...
Độ rộng của khoảng này (trải từ tương quan yếu đến rất mạnh) tự nó
mang thông tin: nó định lượng, thay vì chỉ khẳng định suông, mức độ
bất định ở quy mô pilot đã nêu ở Mục~\ref{sec:threats} — cần một tập
stimulus lớn hơn trước khi coi $\rho=0,605$ là 1 effect size chính
xác.
...
nước tại $\lambda=1,0$). Một biện pháp giảm nhẹ tự nhiên là giới hạn $\mathrm{bonus}$ ở một
mức tối đa cố định,
$\mathrm{bonus}_c(\mathrm{path}(n)) = \min(\mathrm{bonus}(\mathrm{path}(n)),
c)$ cho hằng số $c$ nào đó, giúp chặn độ đánh giá thấp bổ sung tối đa ở
mức cố định $\lambda \cdot c$ bất kể độ sâu đường đi -- chúng tôi đã cài
đặt và thử nghiệm điều này thay vì chỉ dừng ở mức đề xuất.

\paragraph{Thực nghiệm capped-bonus.} Chúng tôi chạy lại đúng tập $N=60$
scramble của Experiment~2 (seed 2000--2059, cùng ngân sách
40.000 node/độ sâu) với $\mathrm{bonus}_c$ cho $c \in \{1, 2, 4\}$ tại
$\lambda \in \{0,5; 1,0\}$, đối chiếu theo cặp với lần chạy không cap đã
báo cáo ở trên. Bảng~\ref{tab:capped-bonus} tóm tắt kết quả (Wilcoxon
signed-rank trên các scramble được giải bởi cả hai điều kiện).
\begin{table}[h]
\centering
\footnotesize
\caption{Capped bonus so với không cap, $N=60$, Wilcoxon signed-rank
theo cặp trên scramble được giải bởi cả hai, đã hiệu chỉnh
Holm--Bonferroni trên toàn bộ $m=10$ kiểm định trong bảng này}
\label{tab:capped-bonus}
\begin{tabular}{ccccc}
\toprule
$\lambda$ & $c$ & giải được & số nước ($p$,$r$) & trig.ov. ($p$,$r$) \\
\midrule
0,5 & 1 & 53/59 & 25,9/26,3 ($p{=}{,}16$) & ,110/,186 ($p{<}10^{-4}$,,57)$^\dagger$ \\
0,5 & 2 & 53/59 & 25,8/26,3 ($p{=}{,}06$) & ,124/,186 ($p{<}10^{-3}$,,54)$^\dagger$ \\
0,5 & 4 & 59/59 & 26,7/26,7 (0/59 khác) & ,201/,201 (0/59 khác) \\
1,0 & 1 & 53/60 & 25,8/27,3 ($p{=}{,}003$,,41)$^\dagger$ & ,124/,248 ($p{<}10^{-6}$,,70)$^\dagger$ \\
1,0 & 2 & 58/60 & 26,7/27,5 ($p{=}{,}03$,,29) & ,199/,253 ($p{=}{,}008$,,35)$^\dagger$ \\
1,0 & 4 & 59/60 & 27,6/27,5 ($p{=}{,}18$; 2/59 khác) & ,256/,256 ($p{=}1{,}0$; 3/59 khác) \\
\bottomrule
\end{tabular}
\\[2pt]
{\scriptsize giải được = capped/uncapped, cả hai trên tổng 60; cột số
nước và trig.ov.\ là trung bình capped/uncapped (trên tập con giải
được ở cả hai điều kiện). $^\dagger$ có ý nghĩa ở mức family-wise
$\alpha=0,05$ sau hiệu chỉnh Holm--Bonferroni trên 10 kiểm định có
$p$-value xác định trong bảng (2 ô tại $\lambda{=}0,5,c{=}4$ có
phương sai bằng 0, không tính vào nhóm hiệu chỉnh). "$X/Y$ khác" làm
rõ: 1 ô không có ý nghĩa hoặc không gắn dấu vẫn có thể chứa khác biệt
thật ở từng scramble nhưng triệt tiêu nhau khi lấy trung bình -- chỉ
riêng hàng $\lambda{=}0,5,c{=}4$ là thực sự giống hệt không cap.}
\end{table}
Hai phát hiện nổi bật. Thứ nhất, $c=4$ không khác biệt về mặt thống kê
so với không cap trên mọi chỉ số ở cả hai giá trị $\lambda$, trong dải
độ dài lời giải này (F2L khoảng 20--33 nước), và vẫn đứng vững sau khi
hiệu chỉnh Holm--Bonferroni cho toàn bộ 10 kiểm định: tại $\lambda=0,5$
đây là khác biệt 0/59 thực sự, còn tại $\lambda=1,0$ có vài scramble
khác nhau (2--3/59) nhưng triệt tiêu nhau khi lấy trung bình, không để
lại hiệu ứng đo được. Dù theo cách nào, cận cố định $\lambda \cdot c$
đạt được \emph{miễn phí}, không tốn chi phí đo lường được nào cho phần
tăng trigger-overlap đã báo cáo ở trên. Thứ hai, và bất ngờ hơn, các
cap chặt hơn ($c=1,2$) \emph{không} đánh đổi trigger-overlap lấy số
nước ít hơn một cách sạch sẽ như kỳ vọng ban đầu -- mọi khác biệt
trigger-overlap trong Bảng~\ref{tab:capped-bonus} đều đứng vững sau
hiệu chỉnh, nhưng chỉ 1 so sánh số nước cực đoan nhất
($\lambda=1,0,c=1$) đứng vững; phần còn lại không phân biệt được với
nhiễu sau hiệu chỉnh. Thay vào đó, cap chặt hơn làm giảm đo được
\emph{tỷ lệ giải thành công} (53--58/60 so với 59--60/60 trong cùng
ngân sách node). Kiểm tra từng scramble cụ thể (ví dụ seed 2053, 2054,
2058) xác nhận cơ chế: với các scramble này, tìm kiếm bị timeout khi
$c=1$ hoặc $c=2$ nhưng giải được khi $c=4$ hoặc không cap -- cho thấy
bonus trigger ở đây không chỉ đơn thuần là một thiên lệch tính-người
mang tính thẩm mỹ, mà còn đóng vai trò \emph{định hướng thứ tự tìm
kiếm} không chính thức, đôi khi giúp tìm kiếm bị giới hạn ngân sách
node thoát khỏi vùng cây không hiệu quả. Vì vậy, giới hạn bonus quá
chặt có thể phản tác dụng trên cả hai trục (tính-người \emph{và} độ
tin cậy) mà không chắc chắn đổi lại được hiệu quả số nước, đây là lý
do chúng tôi báo cáo đây như một đánh đổi thực nghiệm thay vì đề xuất
một giá trị $c$ "an toàn" duy nhất tiên nghiệm.
...
\begin{itemize}
  ...
  \item \textbf{Construct validity của HLI}: chỉ số này được thiết kế
    ...
    qua Experiment~3 (Mục~\ref{sec:exp3}). Chúng tôi cũng chạy thêm 1
    kiểm định đối nghịch (adversarial sanity check): chèn thêm các cặp
    nước "trung tính" (1 trigger đã biết theo ngay sau bởi nghịch đảo
    của chính nó, vd $R\,U\,R'\,U'$ rồi $U\,R\,U'\,R'$, triệt tiêu về
    identity -- đã xác minh bằng tính toán) vào 1 lời giải vốn không
    đổi khác làm Trigger-overlap tăng vọt (vd $0,333 \to 0,706$ sau
    khi chèn 10 cặp no-op vào 1 ví dụ dài 63 nước), vì cả Segmentability
    lẫn Pattern-conformity đều không phạt số nước dư thừa không tạo ra
    tiến triển. Đây là 1 giới hạn thật của Trigger-overlap theo định
    nghĩa hiện tại -- nó không bền vững trước input bị chèn đối
    nghịch. Tuy nhiên, điều này \emph{không} làm suy giảm các kết quả
    đã báo cáo ở trên: Trigger-Biased Search (Nhóm A/A+) được dẫn dắt
    bởi A* tối thiểu hóa số nước thật $g(n)$ cộng với 1 bonus heuristic
    \emph{bị chặn} ở mức $\lambda c$ (Mục~\ref{sec:pathological}), nên
    không có động cơ chèn thêm nước làm tăng chi phí để đổi lấy 1
    phần thưởng không giới hạn/không bị chặn; Nhóm~B đến từ 1 solver
    tối ưu độc lập; Nhóm~H là ghi nhận từ người thật. Công việc tương
    lai áp dụng HLI ngoài bối cảnh tối thiểu-hóa-số-nước này (vd làm
    mục tiêu tối ưu hóa trực tiếp) nên bổ sung 1 số hạng phạt hiệu
    quả/số nước rõ ràng vào Trigger-overlap để khép lại lỗ hổng này.
...
F.~Agostinelli, S.~McAleer, A.~Shmakov, và P.~Baldi.
...
\newblock \url{https://jperm.net/3x3/fingertricks}, truy cập ngày 17 tháng 8 năm 2026.
...
\bibitem[{\'S}wiechowski và {\'S}l{\k{e}}zak(2025)]{humanlike2025}
...
\newblock Trong \emph{Proceedings of the IEEE International Conference on
  Healthcare Informatics (ICHI)}, trang 251--260, 2025.
\newblock Cũng có trên arXiv preprint arXiv:2511.01683.
...
\texttt{research/finger\_tricks.py}. Các tên gọi và chuỗi nước này là
thuật ngữ chuẩn của cộng đồng speedcubing, không phải đóng góp gốc của
công trình này; xem \citet{jperm-fingertricks} như một nguồn tham
khảo tiêu biểu của cộng đồng. Mỗi trigger được liệt kê dưới
