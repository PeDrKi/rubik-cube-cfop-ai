...
nghiệm (N=100 cho so sánh cơ sở, N=60 cho quét Pareto) cho thấy việc
...
validity của HLI (Spearman $\rho=0,487$, $p=0,031$ so với điểm đánh giá
...
\todo{Đoạn này tóm tắt khoảng trống nghiên cứu -- xem phần Related Work
bên dưới, viết lại ngắn gọn ở đây, kết bằng câu dẫn vào contributions}
...
tôi dùng làm từ vựng lý thuyết cho bài báo này. \todo{Nếu tìm thêm được
bài liên quan (vd Zhang et al. 2017 về legibility trong lập kế hoạch
tác vụ robot), thêm 1-2 câu ở đây.}
...
\todo{1 câu về kết quả BotPrize 2012 (UT\^{}2, MirrorBot vượt qua
ngưỡng) -- đã có dữ liệu từ tìm kiếm web ở bước trước, điền vào đây,
PARAPHRASE không trích nguyên văn}. Một khảo cứu gần đây
...
vào tìm kiếm không cấu trúc. \todo{Trích số liệu: X/Y case PLL đã kiểm
chứng, bằng chứng minh bằng \texttt{verify\_and\_build\_table()}}.
...
\todo{X}). \todo{GHI RÕ trong bài: đây là kho hạt giống thủ công, đề
xuất mở rộng bằng thống kê n-gram trên log solve người thật (vd
CSTimer) ở bản mở rộng.}
...
seed 1000--1099 cho Experiment~1, 2000--2059 cho Experiment~2) để tái
lập được. \todo{Nếu chuyển sang máy mạnh hơn trước khi công bố chính
thức, cập nhật lại thông số và CHẠY LẠI toàn bộ thí nghiệm -- tỷ lệ
timeout 22\% có thể giảm đáng kể trên máy nhiều core/nhanh hơn, nên
KHÔNG dùng nguyên số liệu này nếu đổi phần cứng.}
...
Bảng~\ref{tab:exp1} báo cáo kết quả trên $N=100$ scramble random-move
(20 nước mỗi scramble, seed 1000--1099). Nhóm A dùng ngân sách node F2L
đã giảm (40.000 node/độ sâu, từ mặc định 120.000) và timeout 20 giây
thực cho mỗi lần giải để phù hợp với phần cứng đánh giá đơn-lõi của
chúng tôi; $22\%$ scramble ($22/100$) vượt quá timeout này và bị loại
khỏi thống kê theo cặp bên dưới (xem Mục~\ref{sec:pathological} để
thảo luận về hiện tượng này). Trong số $78$ scramble được cả hai nhóm
giải, kiểm định Wilcoxon signed-rank xác nhận Nhóm A cần số nước nhiều
hơn có ý nghĩa so với Nhóm B ($W=0$, $p=1,65\times10^{-14}$, $r=0,87$,
effect size rất lớn), trong khi theo cấu trúc chỉ Nhóm A có HLI khác 0.

\begin{table}[h]
\centering
\caption{Nhóm A (AI-CFOP) so với Nhóm B (Kociemba), N=100 scramble, 78
cặp thành công (Wilcoxon $p=1,65\times10^{-14}$, $r=0,87$ cho khác
biệt số nước)}
\begin{tabular}{lcc}
\toprule
 & Nhóm A (AI-CFOP) & Nhóm B (Kociemba) \\
\midrule
Số nước TB (sd) & 67,2 (11,1) & 20,6 (1,1) \\
Khoảng số nước & 47--105 & 16--22 \\
HLI TB (sd) & 0,655 (0,096) & 0 (theo định nghĩa) \\
Tỷ lệ giải thành công & 78\% (22\% timeout) & 100\% \\
\bottomrule
\end{tabular}
\label{tab:exp1}
\end{table}
...
\todo{Điều tra sau: có thể đây là trùng hợp ngẫu nhiên với N=60, nên
làm lại với N lớn hơn và nhiều mức lambda hơn (0,25, 0,75...) để xác
nhận xu hướng này ổn định, trước khi khẳng định chắc chắn trong bản
...
Trong Experiment 1, $22\%$ scramble ($22/100$) không hoàn thành trong
...
cơ chế thiên vị trigger ở Mục~\ref{sec:method}. Lưu ý Experiment~1 dùng
...
\begin{itemize}
  \item \textbf{Tìm kiếm không hội tụ}: như đã báo cáo ở
    Mục~\ref{sec:pathological}, $22\%$ scramble trong thiết lập phần
    cứng/ngân sách đánh giá của chúng tôi không hoàn thành trong
    timeout 20 giây; kết quả có điều kiện trên $78\%$ đã hoàn thành, có
  ...
  \item \textbf{Thiên vị kho ngữ liệu}: kho finger-trick $T$ được tuyển
    ...
    trigger khác.
  \item \textbf{Sinh scramble (đã kiểm tra)}: chúng tôi đã cài đặt một
    bộ sinh scramble kiểu WCA random-state (Mục~\ref{sec:setup}) và
    chạy lại Experiment~1 trên $N=60$ scramble random-state như một
    kiểm tra độ mạnh mẽ (robustness check). Kết quả nhất quán với điều
    kiện random-move (80\% so với 78\% tỷ lệ giải thành công; số nước
    trung bình 66,3 so với 67,2; HLI trung bình 0,658 so với 0,655),
    cho thấy kết quả random-move ban đầu của chúng tôi không phải là
    hiện tượng giả tạo do scramble dễ hơn một cách có hệ thống. Dù vậy
    chúng tôi báo cáo minh bạch kết quả từ cả hai bộ sinh
    (Bảng~\ref{tab:exp1} dùng random-move để nhất quán với
    Experiment~2; \todo{nếu muốn, đổi Bảng 1 chính sang dùng
    random-state làm kết quả chính, giữ random-move như robustness
    check phụ -- cả 2 cách đều chấp nhận được, tùy quyết định}).
...
\todo{ý tưởng: xây dựng bảng case F2L 41-case chuẩn (như OLL/PLL đã
có), thay thế một phần trigger-overlap bằng pattern-conformity thật sự
cho F2L.}

\paragraph{Cơ chế thiên vị thay thế.}
\todo{So sánh trigger-biased A* với cách khác, vd fine-tune trọng số
PDB, hoặc học trọng số bonus từ dữ liệu solve người (thay vì $\lambda$
cố định tay).}
...
nghiệm ($N=100$ scramble) rằng solver AI-CFOP của chúng tôi đạt điểm
cao hơn đáng kể trên chỉ số này so với baseline Kociemba tối-ưu-kiểu-máy-tính
(HLI trung bình $0,655$ so với $0$ theo định nghĩa), một khoảng cách
...
\newblock \url{http://kociemba.org/cube.htm}, truy cập \todo{ngày truy cập}.
