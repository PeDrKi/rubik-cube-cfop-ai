...
solver has since been extended to a verified single-look table
covering 55 of the 57 OLL cases (43 confirmed against the standard
community numbering, the remainder auto-derived and self-verified
via exhaustive search); the 2 unresolved cases (OLL~2 and OLL~20)
provably admit no pure face-turn algorithm shorter than 13 moves
(established by bidirectional meet-in-the-middle search over the
full cube-state graph) and fall back to the same verified 2-look
decomposition. This upgrade postdates data collection and does not
alter the results reported in Section~\ref{sec:results}, which
reflect the 2-generator OLL table described above.
...
the subsequent 55/57 single-look upgrade, which does not affect the
...
\begin{itemize}
  ...
  \item \textbf{Inter-rater reliability}: individual raters showed
    ...
    single rater's judgment. \emph{Note: these ICC figures are
    reported from the survey write-up and have not yet been
    independently re-derived from the raw per-rater rating matrix in
    this repository; re-verify before a formal submission.}
