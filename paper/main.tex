...
human finger-tricks. Pilot experiments (N=100 for the baseline comparison, N=60 for the
Pareto sweep) show that biasing the F2L search heuristic increases
...
compute time), Pareto trade-off. We further outline a BotPrize-style
\citep{hingston2009} human-judge validation protocol to establish
construct validity of HLI. This work is, to our knowledge, the first
...
\todo{Doan nay tom tat khoang trong nghien cuu -- xem phan Related Work
ben duoi, viet lai ngan gon o day, ket bang cau dan vao contributions}
...
\begin{enumerate}
  ...
  \item A proposed, BotPrize-style \citep{hingston2009} human-judge
    validation protocol for construct-validating HLI
    (Section~\ref{sec:future}), positioning this work for extension
    into a full empirical study.
\end{enumerate}
...
theoretical vocabulary for this paper. \todo{Neu tim them duoc bai
lien quan (vd Zhang et al. 2017 ve legibility trong robot task
planning), them 1-2 cau o day.}
...
human-likeness scale, without being told which is which. \todo{1 cau
ve ket qua BotPrize 2012 (UT\^{}2, MirrorBot vuot qua nguong) -- da co
du lieu tu tim kiem web o buoc truoc, dien vao day, PARAPHRASE khong
trich nguyen van}. A recent survey \citep{humanlike2025} frames
...
--- and (b) mechanism --- an explicit, tunable search bias
...
unstructured search fallback. \todo{Trich so lieu: X/Y PLL case
verified, bang chung minh bang \texttt{verify\_and\_build\_table()}}.
...
Appendix~\todo{X}). \todo{GHI RO trong bai: day la seed corpus thu
cong, de xuat mo rong bang thong ke n-gram tren log solve nguoi that
(vd CSTimer) o ban mo rong.}
...
\textbf{Scramble generation.} Primary results use random-move
scrambles (20 moves, no immediately-repeated face) for consistency
across Experiments~1--2. We additionally implemented a WCA-style
\emph{random-state} scrambler (long random walk to approximate a
uniform state distribution, followed by inverting a near-optimal
Kociemba solve of that state --- guaranteeing the scramble requires
close to the optimal move count to undo, matching real competition
scrambler methodology) and validated that results are consistent
between the two generators (Section~\ref{sec:discussion}).
...
(\texttt{random.Random(seed)}, seeds 1000--1099 for Experiment~1,
2000--2059 for Experiment~2) for reproducibility. \todo{Neu chuyen
sang may manh hon truoc khi cong bo chinh thuc, cap nhat lai thong so
va CHAY LAI toan bo thi nghiem -- ty le timeout 22\% co the giam dang
ke tren may nhieu core/nhanh hon, nen KHONG dung nguyen so lieu nay
neu doi phan cung.}
...
Table~\ref{tab:exp1} reports results on $N=100$ random-move scrambles
(20 moves each, seeds 1000--1099). Group A used a reduced F2L node
budget (40{,}000 nodes/depth, down from the 120{,}000 default) and a
20-second wall-clock timeout per solve to accommodate our
single-core evaluation hardware; $22\%$ of scrambles ($22/100$)
exceeded this timeout and are excluded from the paired statistics
below (see Section~\ref{sec:pathological} for discussion of this
phenomenon). Among the $78$ scrambles solved by both groups, a
Wilcoxon signed-rank test confirms Group A requires significantly
more moves than Group B ($W=0$, $p=1.65\times10^{-14}$,
$r=0.87$, a very large effect), while by construction only Group A
has non-zero HLI.

\begin{table}[h]
\centering
\caption{Group A (AI-CFOP) vs Group B (Kociemba), N=100 scrambles,
78 paired successes (Wilcoxon $p=1.65\times10^{-14}$, $r=0.87$ for
move-count difference)}
\begin{tabular}{lcc}
\toprule
 & Group A (AI-CFOP) & Group B (Kociemba) \\
\midrule
Mean move count (sd) & 67.2 (11.1) & 20.6 (1.1) \\
Move count range & 47--105 & 16--22 \\
Mean HLI (sd) & 0.655 (0.096) & 0 (by definition) \\
Solve success rate & 78\% (22\% timeout) & 100\% \\
\bottomrule
\end{tabular}
\label{tab:exp1}
\end{table}
...
\todo{Dieu tra sau: co the day la trung hop ngau nhien voi N=60, nen
lam lai voi N lon hon va nhieu muc lambda hon (0.25, 0.75...) de xac
nhan xu huong nay on dinh, truoc khi khang dinh chac chan trong ban
cong bo chinh thuc.} A formal analysis of exactly how much
...
During Experiment 1, $22\%$ of scrambles ($22/100$) failed to
...
mechanism of Section~\ref{sec:method}. Note that Experiment~1 used
...
\begin{itemize}
  \item \textbf{Search non-convergence}: as reported in
    Section~\ref{sec:pathological}, $22\%$ of scrambles in our
    evaluation hardware/budget setting did not complete within a
    20-second timeout; results are conditioned on the $78\%$ that
    did, which may not be a representative subsample (e.g.\ if
    difficulty correlates with trigger-overlap potential).
  \item \textbf{Construct validity of HLI}: the metric is
    hand-designed, not yet validated against human judgment ---
    addressed by the proposed Phase~3 protocol
  ...
  \item \textbf{Corpus bias}: the finger-trick corpus $T$ is
    ...
    vocabularies.
  \item \textbf{Scramble generation (checked)}: we implemented a
    WCA-style random-state scrambler (Section~\ref{sec:setup}) and
    re-ran Experiment~1 on $N=60$ random-state scrambles as a
    robustness check. Results were consistent with the random-move
    condition (80\% vs.\ 78\% solve rate; mean moves 66.3 vs.\ 67.2;
    mean HLI 0.658 vs.\ 0.655), suggesting our original random-move
    results were not an artifact of systematically easier scrambles.
    We nonetheless report results from both generators transparently
    (Table~\ref{tab:exp1} uses random-move for consistency with
    Experiment~2; \todo{neu muon, doi Table 1 chinh sang dung
    random-state lam ket qua chinh, giu random-move nhu robustness
    check phu -- ca 2 cach deu chap nhan duoc, tuy quyet dinh}).
...
\paragraph{Phase 3: Human-judge validation (BotPrize-style).}
Following \citet{hingston2009}, we plan a blinded study in which
CFOP-literate participants rate solve sequences (sourced from real
human solves, Group A, Group A+trigger-bias, and Group B) on a
1--5 human-likeness scale, and correlate ratings with automated HLI
via Spearman's $\rho$, to establish construct validity.

\paragraph{Extending pattern-conformity to Cross/F2L.}
\todo{y tuong: xay dung bang case F2L 41-case chuan (nhu OLL/PLL da
co), thay the mot phan trigger-overlap bang pattern-conformity that
su cho F2L.}

\paragraph{Alternative bias mechanisms.}
\todo{So sanh trigger-biased A* voi cach khac, vd fine-tune trong so
PDB, hoac hoc trong so bonus tu du lieu solve nguoi (thay vi $\lambda$
co dinh tay).}
...
(HLI), and showed empirically ($N=100$ scrambles) that our
AI-CFOP solver scores substantially higher on this index than a
computer-optimal Kociemba baseline (mean HLI $0.655$ vs.\ $0$ by
...
limitation of penalty-augmented heuristic search in this domain. We
outlined, and partially instrumented (Section~\ref{sec:future}), a
BotPrize-style human-judge validation protocol as the natural next
step toward establishing HLI's construct validity beyond
proxy-metric agreement. We hope this work encourages further
...
\newblock \url{http://kociemba.org/cube.htm}, accessed \todo{ngay truy cap}.
